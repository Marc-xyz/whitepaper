...
Las plataforma de contratos inteligentes (_smart contracts_) y las criptomonedas han suscitado la fascinación de las masas, aún así no han podido lograr una gran adopción por estas debido a sus inherentes problemáticas de escalabilidad y experiencia relativamente sofisticada para el usuario medio. Incluso en Ethereum, plataforma de contratos inteligentes más empleada, no han habido muchos ejemplos de aplicaciones descentralizadas (_Dapps_) que hayan llevado a esta adopción masiva. A pesar de ello ha habido algunos pocos casos que temporalmente alguna u otra aplicación en concreto a conseguido captar una base de usuarios significativa, es decir tener un cierto éxito, pero a pesar de ello y por ese mismo motivo conducir a la propia red (_network_) a la saturación en tiempos de alta actividad en la misma. En consecuencia, esto nos da evidencias, que incluso las plataformas más punteras del momento aún no están listas para gestionar la actividad que generaría la adopción masiva de estas tecnologías. 
<br/><br/>

<b>La red (_network_) de Matic (ahora Polygon) se esfuerza por resolver los problemas de escalabilidad y usabilidad, sin comprometer ,claro esta, con la descentralización y aprovechando tanto la comunidad de desarrolladores preexistente así como todo su ecosistema. Es una solución de escalabilidad del <200b>off/side para plataformas existentes que además mejora la experiencia anterior del usuario.</b>
<br/><br>  
  
<b>El equipo de desarrollo de Matic (ahora Polygon) ha elegido Ethereum como la primera plataforma para mostrar su posible escalabilidad; ya tiene una primera implementación de trabajo para Ethereum en Kovan Testnet.</b> Se espera que esta implementación permita realizar transferencias, intercambios y conversiones casi instantáneas de activos digitales (por ejemplo tokens) y criptomonedas en un futuro cercano. La red de Matic es una implementación del _framework_ Plasma de Ethereum para comenzar, pero la "visión" de el equipo desarrollador de Matic es proporcionar soluciones de escalabilidad para cadenas de bloques en general. La fundación Matic tiene la intención de proporcionar billeteras Matic, pagos de APIs & SDKs, productos, soluciones de identidad y otras soluciones habilitadoras que permitirán a las personas desarolladoras diseñar, implementar y migrar_DApps_  construidas en plataformas base como no, como Ethereum. Uno de los puntos claves que forman parte de la ideología de la red Matic es el compromiso con la mejora de la experiencia de usuario, consideramos que esta área de trabajo esta poco desarrollada para las aplicaciones de cadenas de bloques por el momento. El equipo de personas detrás de Matic ya han creado una mejora en la experiencia de usuario de alta calidad, mediante librerías de navegadores móviles/web que permiten a las empresas crear aplicaciones de usuario final del mundo real a gran escala. La hoja de ruta de desarollo de la red Matic también incluye el apoyo a las transferencias entre cadenas de bloques distintas, las transferencias con _exchanges_ descentralizados de terceros,piscinas de liquidez, etc.
</div>

# ¿Por qué Matic? {#whymatic}

Las aplicaciones descentralizada prosperan en gran gran cantidad, pero el ecosistema de cadenas de bloques (_blockchains_) aún no esta preparada para satisfacer la estabilidad necesaria para la adopción masiva por parte del usuario final. Además se debe destacar que para el mismo la experiencia en el uso de DApps es muy pobre y de ninguna manera accesible al usuario promedio. Las lentas confirmaciones de bloque, las altas tarifas o comisiones de transacción, la baja escalabilidad  y la comentada mala experiencia para la persona usuario final son algunos de los obstáculos clave para la adopción masiva de aplicaciones basadas en cadenas de bloques. La siguiente sección explica los problemas recurrentes en el ecosistema de las cadenas de bloques y cómo la red Matic pretende resolverlos. **Las especificaciones técnicas detalladas se proporcionan en las secciones futuras del presente _whitepaper_.**

### Transacciones lentas  {#slowt}

Las tracciones en cadenas de bloques (_blockchain_) típicamente son muy lentas y tienen un rendimiento francamente limitado. La mayoría de PoW([Proof-of-Work](https://en.wikipedia.org/wiki/Proof-of-work_system)) protocolos basados en prueba de trabajo (_Proof of work_)tienen un limite en el tamaño de sus bloques y toma un cierto tiempo generar cada nuevo bloque de la cadena de bloques. Además cada transacción tiene que esperar múltiples confirmaciones  de bloque debido a las posibles reorganizaciones de la cadena (lo que implica más tiempo).<br/>

Las cadenas de bloques basadas en prueba de participación o de deposito (_PoS_) ([Proof-of-Stake](https://github.com/ethereum/wiki/wiki/Proof-of-Stake-FAQs#what-is-proof-of-stake)) intentan contrarrestar estas limitaciones utilizando un mecanismo de participación o deposito, no obstante las cadenas de bloques basadas en prueba de participación  (_PoS_) pueden lograr un alto rendimiento, aunque normalmente, a costa de limitar la descentralización. Estas limitaciones suelen ser una condición necesaria para que las cadenas de bloques públicas puedan garantizar la seguridad y la descentralización cuando un bloque deba propagarse a través de la red y ser validado por todos los nodos para finalmente ser aceptado en la cadena de bloques.<br/>

La red Matic resuelve este problema mediante el uso de una cadena de bloques de alto rendimiento con el consenso proporcionado por un conjunto seleccionado de productores de bloques , elegidos para cada verificación por un conjunto de participantes (_stakers_). Después es usada una capa (_layer_) de _PoS_ para **validar  los bloques y publicar las pruebas periódicas (_merkle roots_) de los bloques de la cadena producidos  por los productores de bloques en la cadena principal  (_mainchain_) de Ethereum. Esto ayuda a lograr un alto nivel de descentralización mientras mantiene tiempos de confirmación de bloque suficientemente rápidos (menores a dos segundos)**

### Bajo rendimientoglobal {#ltt}

Las cadenas de bloques públicas deben mantener una cierta cantidad de tiempo entre la producción de bloques adyacentes para garantizar un tiempo suficiente para la propagación del bloque a los diferentes nodos. Además el tamaño del bloque debe ser pequeño para garantizar una rápida propagación del bloque a través de la red. Esto implica que el número de transacciones de un bloque dado debería ser bastante limitado para no incurrir en la posible anterior problemática. <br/>


La red Matic  resuleve esta problemática usando **una capa (_layer_) de Productores de bloques (_Blocke Producers_) para producir los bloques de la cadena.**  Los productores de bloques permiten al sistema producir bloques de la cadena de forma muy rápida (suficientemente veloz). El sistema asegura la descentralización utilizando puntos de control de  Prueba de Participación (_PoS_) que se envían a la cadena principal  (Mainchain) de Ethereum  (Los servidores de Ethereum sirven como la cadena principal (_Mainchain_) en el inicio). **Esto mismo permite que la red de Matic , al menos teóricamente , logre hasta ![TEX](https://render.githubusercontent.com/render/math?math=2^16) transacciones por segundo en una simple cadena lateral.**

### Escalabilidad {#scalability}

Tal como se discutió en la sección anterior, la red de Matic alcanza fácilmente una velocidad teórica de hasta 2^ {16}  transacciones por segundo en una sola cadena lateral. **En el futuro, se espera que _The Matic Network_ pueda agregar fácilmente más cadenas laterales horizontalmente para incrementar el número total de transacciones en la cadena Matic mientras usa la misma capa de _PoS_ descentralizada.**

En teoría, _Matic Network_ (o la red Matic) tiene capacidad para millones de transacciones por segundo con el uso de múltiples cadenas laterales. Además, el mecanismo para hacerlo posible ha sido demostrado con la primera prueba de concepto (_proof-of-concept_) de Matic con la primera cadena lateral (_side-cahin_) de Matic y en a su debido tiempo se podrán agregar nuevas cadenas de bloques.

### Tamaño de la cadena de bloques {#sob}

Cada bloque en la cadena de bloques y/o estado de cálculo en el caso de una cadena de bloques basada en un contrato inteligente debe ser validado por varios nodos. Cada nodo debe gestionar una copia del estado y los bloques. Si bien la cadena de bloques aumenta de tamaño a medida que pasan los días (el tiempo), mantener y validar toda la cadena de bloques se vuelve difícil y da como resultado menos nodos completos en las cadenas de bloques públicas, lo que representa un riesgo de descentralización.<br/>

Para la _Matic Network_, la capa primaria  que proporciona descentralización puede optar por almacenar solo los bloques de _Matic Chain_ desde el punto de control anterior (_previous checkpoint_) al siguiente punto de control (_next checkpoint_). Todas las pruebas de transacciones / bloques anteriores se han enviado a la cadena principal (_mainchain_). **Esto permite nodos de PoS de muy baja fidelidad que se pueden ejecutar en máquinas de muy bajo costo con poco almacenamiento. En el futuro, The Matic Network también tiene la intención de habilitar mineros de PoS basados en dispositivos móviles.**

###  Múltiples canales de micropagos con otras soluciones fuera de la cadena de bloques Matic {#loooong}

Algunas soluciones de canales de pago han propuesto soluciones para resolver el problema de los micropagos (_micro-payments_). Sin embargo, el proceso de apertura y gestión de canales con múltiples _DApps_  (_aplicaciones descentralizadas_) o usuarios es complejo. Adicionalmente, la velocidad y la conveniencia de los pagos con mediación a través de canales aún son objeto de debate.


Dado que **_The Matic Network_ utiliza una arquitectura basada en el estado en una EVM (máquina virtual Ethereum), no requiere que se abran canales de pago entre dos partes. De hecho, cualquier dirección Ethereum válida es una dirección Matic válida y un receptor no necesita estar en la cadena Matic para recibir el pago. Solo necesitarían tener una billetera Matic cuando quiera recuperar los pagos en la cadena principal o gastarlos en el ecosistema en la red Matic.**


### Altas comisiones por transacción  {#htf}

Con el rápido crecimiento del ecosistema blockchain, cada vez más, se crean, transfieren y venden nuevos criptoactivos, que a menudo involucran múltiples cripto tokens. Además, la mayoría de las aplicaciones descentralizadas tienen su propio token y economía. Pagar tokens por los servicios o realizar cualquier tipo de transacción en _blockchains_ requiere transferencias en la  cadena. Cada cadena de bloques tiene una estructura de costos de transacción. Por ejemplo, Ethereum cobra tarifas de _gas_ en cada transacción.

La cantidad en concepto de comisión (_fee_s) es un factor importante para incentivar a los validadores y prevenir ciertos tipos de ataques de seguridad como _DoS_ (ataque distribuido de denegación de servicios). Sin embargo, existe el problema de la variación de las comisiones por realizar una transacción en la cadena de bloques (según el grupo de transacciones pendientes) debido al tamaño limitado del bloque.

**La red Matic permite transacciones de bajo costo mediante el logro de economías de escala  (_achieving economies of scale_) al realizar una gran cantidad de transacciones en la capa Block Producer (_Block Producer layer_), lo que garantiza un bajo costo, y luego agrupando posteriormente las pruebas de los bloques Matic utilizando la raíz Merkle de los bloques (_the Merkle root of the blocks_)** a una cadena principal altamente descentralizada (por ejemplo, Ethereum) usando una capa descentralizada de _PoS Stakers_ (Prueba de deposito o participación).

### Escasa usabilidad {#pf}

Las interacciones del usuario en las _DApps_ (aplicaciones descentralizadas) suelen ser deficientes en comparación con sus contrapartes centralizadas. Para que la revolución de la descentralización logre una adopción masiva, la experiencia del usuario usando _DApps_ debe estar a la par, si no mejor, que sus homólogos centralizadas.

Se espera que el equipo de desarrollo de Matic trabaje en varias herramientas de integración de navegadores web y móviles y es pionero en protocolos en este campo. Tiene la intención de construir una aplicación de navegador web/móvil ubicua, que actuará como una capa de interacción segura para las interacciones con las cadenas de bloques. El equipo de desarrollo de Matic publicará los diseños y prototipos de estos pronto.
